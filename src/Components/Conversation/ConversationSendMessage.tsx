...
const ConversationSendMessage = ({ socket }: any) => {
  const token = Cookies.get("host.bonavent_accessToken");
  const selectedConversation: IConversation | null = useAppSelector(selectSelectedChatUser);
  ...
  const uploadImage = async (file: any) => {
    ...
    formData.append("uploadChatDocument", file.originFileObj);

    try {
      const response = await axios.post(
        `${serverUrl}/chat/upload-chat-document`,
        formData,
        {
          headers: {
  ...
  const handleMessageSend = async (values: any) => {
    const data: any = {
      targetId: selectedConversation?.guestId,
      ...(values?.message?.length > 0 ? { text: values?.message } : {}),
      ...(uploadedImages.length > 0 ? { file: uploadedImages } : {}),
      fromDashBoard: true,
    };
