...
const ConversationMessage = ({ userData, onlineUsers }: any) => {
  ...
  const selectedConversation: IConversation | null = useSelector(selectSelectedChatUser);
  ...
  useEffect(() => {
    selectedConvIdRef.current = selectedConversation?._id ?? null;
  ...
  useEffect(() => {
    ...
    setMessages((prev) => {
      if (page === 1) {
        return allMessages.data.messages;
      } else {
        const newMessages = allMessages.data.messages?.filter(
  ...
  const handleMessage = useCallback((message: any) => {
    console.log(message)
    const currentChatId = selectedConvIdRef.current;

    if (message.chatId === currentChatId) {

      const formattedMessage = {
        _id: message.chatId ?? crypto.randomUUID(),
  ...
  useEffect(() => {
    ...
    socket.emit("hasChatOpen", { chatId: roomId, isOpen: true });
